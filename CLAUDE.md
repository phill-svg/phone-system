...
- **`runbooks/`** — the mechanical procedures, none of which run from CI here:
  - `ios-abm-custom-app.md` — iOS distribution via Apple Business Manager (see status below)
...
- **iOS:** TestFlight external testing was **rejected under Guideline 2.2** — TestFlight is for
  ...
  submission would hit **Guideline 3.2 (Business)** for the same underlying reason. Interim
  distribution is internal TestFlight (no Beta App Review); the durable fix is an Apple Business
  Manager custom app — `runbooks/ios-abm-custom-app.md`. Note the interaction with tenancy
  sub-project 7 documented in that runbook.
