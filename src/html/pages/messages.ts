...
export function renderMessagesPage(role: "admin" | "staff" = "admin"): string {
  ...
  const body =
    '<div class="msg-wrap">' +
      ...
      '</div>' +
    '</div>' +
    '<datalist id="contactNumbers"></datalist>' +
...
const CLIENT_JS = [
  ...
  'function newMessage(){current=null;var head=document.getElementById("paneHead");head.innerHTML="";var lbl=document.createElement("label");lbl.className="to-label";lbl.textContent="To";lbl.setAttribute("for","toInput");var input=document.createElement("input");input.id="toInput";input.type="tel";input.placeholder="Phone number";input.autocomplete="off";input.setAttribute("list","contactNumbers");var hint=document.createElement("span");hint.className="to-hint";hint.id="toHint";head.appendChild(lbl);head.appendChild(input);head.appendChild(hint);fillContactDatalist();var sc=document.getElementById("scroll");sc.innerHTML="";var em=document.createElement("div");em.className="msg-empty";em.textContent="Enter a number, then write your message below.";sc.appendChild(em);document.getElementById("composer").style.display="flex";document.getElementById("text").placeholder="Text message";renderSmsFromRow();input.addEventListener("input",updateToHint);input.addEventListener("keydown",function(e){if(e.key==="Enter"){e.preventDefault();document.getElementById("text").focus();}});input.focus();}',
  // Native <datalist> autocomplete over known contacts -- no custom suggestion widget needed.
  'function fillContactDatalist(){var dl=document.getElementById("contactNumbers");if(!dl)return;dl.innerHTML="";Object.keys(contactsByNorm).forEach(function(k){var c=contactsByNorm[k];if(!c||!c.phone)return;var o=document.createElement("option");o.value=c.phone;o.label=c.name||"";dl.appendChild(o);});}',
