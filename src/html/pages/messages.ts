...
const CLIENT_JS = [
  ...
  'function renderThread(msgs){var el=document.getElementById("scroll");if(msgs.length===0){el.innerHTML="<div class=\\"msg-empty\\">No messages yet. Send the first one below.</div>";return;}var html="";for(var i=0;i<msgs.length;i++){var m=msgs[i];var out=m.direction==="outbound";var inCls=isMessenger(current)?"in fb":"in";html+="<div class=\\"bubble-row "+(out?"out":"in")+"\\"><div class=\\"bubble "+(out?"out":inCls)+"\\">"+esc(m.body)+"</div></div>";}el.innerHTML=html;el.scrollTop=el.scrollHeight;}',
