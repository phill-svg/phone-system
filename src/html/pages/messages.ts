...
export function renderMessagesPage(role: "admin" | "staff" = "admin"): string {
  const style = `<style>
    ...
    .to-label { font-size: 0.85rem; color: var(--admin-dim); }
    .to-hint { font-size: 0.85rem; color: var(--admin-dim); }
    .to-suggest { max-height: 240px; overflow-y: auto; background: var(--admin-surface); border-bottom: 1px solid var(--admin-border); }
    .to-suggest button { display: flex; justify-content: space-between; gap: 0.8rem; width: 100%; text-align: left; background: none; border: 0; border-bottom: 1px solid var(--admin-border); padding: 0.5rem 1.1rem; color: var(--admin-text); cursor: pointer; font-size: 0.9rem; }
    .to-suggest button:last-child { border-bottom: 0; }
    .to-suggest button:hover, .to-suggest button.active { background: var(--admin-surface-hover); }
    .to-suggest .ts-num { color: var(--admin-dim); font-size: 0.82rem; white-space: nowrap; }
    .to-suggest .ts-empty { padding: 0.55rem 1.1rem; color: var(--admin-dim); font-size: 0.85rem; }
  ...
  const body =
    '<div class="msg-wrap">' +
      ...
      '<div class="msg-pane">' +
        ...
        '<div class="to-suggest" id="toSuggest" style="display:none"></div>' +
...
const CLIENT_JS = [
  ...
  // Quotes too: esc() output goes into data-num="..." among others, and textContent -> innerHTML
  // escapes only & < >. See the same fix in ivrFlow.ts's h().
  ...
  // A small inline editor that takes over the thread header. Replaces the browser prompt() this
  // used to use -- a native dialog looks like the browser talking, not the app.
  'function headEditor(labelText,initial,placeholder,onCommit,onCancel){hideToSuggest();var head=document.getElementById("paneHead");if(!head)return;head.innerHTML="";var lbl=document.createElement("span");lbl.className="to-label";lbl.textContent=labelText;var input=document.createElement("input");input.type="text";input.value=initial||"";input.placeholder=placeholder;input.autocomplete="off";var save=document.createElement("button");save.className="fb-name-btn";save.textContent="Save";var cancel=document.createElement("button");cancel.className="fb-name-btn";cancel.textContent="Cancel";var msg=document.createElement("span");msg.className="to-hint";head.appendChild(lbl);head.appendChild(input);head.appendChild(save);head.appendChild(cancel);head.appendChild(msg);function commit(){var v=(input.value||"").trim();if(!v){input.focus();return;}save.disabled=true;msg.textContent="";onCommit(v,function(errText){msg.textContent=errText;save.disabled=false;});}save.addEventListener("click",commit);cancel.addEventListener("click",onCancel);input.addEventListener("keydown",function(e){if(e.key==="Enter"){e.preventDefault();commit();}else if(e.key==="Escape"){onCancel();}});input.focus();input.select();}',
  // Offered on any SMS thread whose number is not already a contact.
  'function addSaveContactButton(number){var head=document.getElementById("paneHead");if(!head)return;var b=document.createElement("button");b.className="fb-name-btn";b.id="saveContactBtn";b.textContent="Save contact";b.title="Add this number to contacts";b.addEventListener("click",function(){saveContactInline(number);});head.appendChild(b);}',
  'function saveContactInline(number){headEditor(formatAu(number),"","Contact name",function(name,fail){api("/api/contacts",{method:"POST",body:JSON.stringify({name:name,phone:number})}).then(function(){return loadContacts();}).then(function(){openThread(number);loadConversations();}).catch(function(e){fail((e&&e.message)||"Could not save that contact.");});},function(){openThread(number);});}',
  ...
  'function nameFbPerson(number){var el=document.querySelector("#paneHead strong");var cur=el?el.textContent:"";if(cur==="Facebook user")cur="";headEditor("Facebook contact",cur,"Name",function(name,fail){api("/api/facebook/name",{method:"PUT",body:JSON.stringify({psid:number,name:name})}).then(function(){loadConversations();openThread(number,name);}).catch(function(){fail("Could not save that name.");});},function(){openThread(number,cur||undefined);});}',
  ...
  'function openThread(number,name){hideToSuggest();current=number;document.getElementById("composer").style.display="flex";renderSmsFromRow();var ta=document.getElementById("text");if(ta)ta.placeholder=isMessenger(number)?"Reply on Messenger":"Text message";var head=document.getElementById("paneHead");var c=isMessenger(number)?null:contactsByNorm[normalizePhoneJS(number)];var disp=name||(isMessenger(number)?"Facebook user":(resolveName(number)||formatAu(number)));if(c){head.innerHTML="<span class=\\"pane-head-name\\" id=\\"paneHeadName\\" title=\\"View contact\\"><strong>"+esc(disp)+"</strong></span>"+chanChip(number);var el=document.getElementById("paneHeadName");if(el)el.addEventListener("click",function(){showContactPreview(c,number);});}else{head.innerHTML="<strong>"+esc(disp)+"</strong>"+chanChip(number);if(!isMessenger(number))addSaveContactButton(number);}if(isMessenger(number))addFbNameButton(number,disp!=="Facebook user");loadThread();loadConversations();}',
  ...
  // A late poll response for a thread no longer open would render under the new thread's header.
  'function loadThread(){if(!current)return;var peer=current;api("/api/messages/"+encodeURIComponent(peer)).then(function(msgs){if(peer!==current)return;renderThread(msgs||[]);}).catch(function(){});}',
  ...
  'function newMessage(){current=null;var head=document.getElementById("paneHead");head.innerHTML="";var lbl=document.createElement("label");lbl.className="to-label";lbl.textContent="To";lbl.setAttribute("for","toInput");var input=document.createElement("input");input.id="toInput";input.type="tel";input.placeholder="Name or phone number";input.autocomplete="off";var hint=document.createElement("span");hint.className="to-hint";hint.id="toHint";head.appendChild(lbl);head.appendChild(input);head.appendChild(hint);var sc=document.getElementById("scroll");sc.innerHTML="";var em=document.createElement("div");em.className="msg-empty";em.textContent="Search a contact or type a number, then write your message below.";sc.appendChild(em);document.getElementById("composer").style.display="flex";document.getElementById("text").placeholder="Text message";renderSmsFromRow();input.addEventListener("input",function(){updateToHint();renderToSuggest(input.value);});input.addEventListener("focus",function(){renderToSuggest(input.value);});input.addEventListener("blur",function(){setTimeout(hideToSuggest,120);});input.addEventListener("keydown",function(e){if(e.key==="ArrowDown"){e.preventDefault();moveToSuggest(1);return;}if(e.key==="ArrowUp"){e.preventDefault();moveToSuggest(-1);return;}if(e.key==="Escape"){hideToSuggest();return;}if(e.key==="Enter"){e.preventDefault();var a=activeToSuggest();if(a){a.dispatchEvent(new MouseEvent("mousedown"));return;}hideToSuggest();document.getElementById("text").focus();}});input.focus();renderToSuggest("");}',
  // Contact search for the To field. This was a native <datalist>, which can only autocomplete
  // on the option's VALUE -- the phone number -- so typing a name matched nothing and clicking
  // the field just dropped every contact in one undifferentiated list. Match on name, company
  // OR digits, and show the name and number together so you can tell two Daves apart.
  'function contactList(){var out=[];Object.keys(contactsByNorm).forEach(function(k){var c=contactsByNorm[k];if(c&&c.phone)out.push(c);});out.sort(function(a,b){return (a.name||"").localeCompare(b.name||"");});return out;}',
  'function searchContacts(q){var list=contactList();var s=(q||"").trim().toLowerCase();if(!s)return list;var digits=s.replace(/[^0-9]/g,"");return list.filter(function(c){if((c.name||"").toLowerCase().indexOf(s)>=0)return true;if((c.company||"").toLowerCase().indexOf(s)>=0)return true;return digits.length>=2&&(c.phone_normalized||"").indexOf(digits)>=0;});}',
  'function hideToSuggest(){var el=document.getElementById("toSuggest");if(!el)return;el.style.display="none";el.innerHTML="";}',
  'function renderToSuggest(q){var el=document.getElementById("toSuggest");if(!el)return;var hits=searchContacts(q).slice(0,60);el.innerHTML="";if(!hits.length){el.innerHTML="<div class=\\"ts-empty\\">No matching contacts</div>";el.style.display="block";return;}hits.forEach(function(c){var b=document.createElement("button");b.type="button";b.innerHTML="<span>"+esc(c.name||formatAu(c.phone))+"</span><span class=\\"ts-num\\">"+esc(formatAu(c.phone))+"</span>";b.addEventListener("mousedown",function(e){e.preventDefault();pickContact(c);});el.appendChild(b);});el.style.display="block";}',
  'function pickContact(c){var input=document.getElementById("toInput");if(!input)return;input.value=c.phone;hideToSuggest();updateToHint();var ta=document.getElementById("text");if(ta)ta.focus();}',
  // Arrow keys move the highlight; Enter takes the highlighted contact, or falls through to the
  // message box when nothing is highlighted (so a typed-out number still works the old way).
  'function moveToSuggest(dir){var el=document.getElementById("toSuggest");if(!el||el.style.display==="none")return null;var btns=el.querySelectorAll("button");if(!btns.length)return null;var cur=-1;for(var i=0;i<btns.length;i++){if(btns[i].className==="active")cur=i;}var next=cur+dir;if(next<0)next=btns.length-1;if(next>=btns.length)next=0;for(var j=0;j<btns.length;j++)btns[j].className="";btns[next].className="active";btns[next].scrollIntoView({block:"nearest"});return btns[next];}',
  'function activeToSuggest(){var el=document.getElementById("toSuggest");if(!el||el.style.display==="none")return null;return el.querySelector("button.active");}',
  // Live feedback while typing a new recipient: name it if we know them, flag it if it is unusable.
  'function updateToHint(){var i=document.getElementById("toInput");var h=document.getElementById("toHint");if(!i||!h)return;var v=(i.value||"").trim();if(!v){h.textContent="";return;}var n=resolveName(v);h.textContent=n?n:(v.replace(/[^0-9]/g,"").length<6?"Keep typing...":"");}',
