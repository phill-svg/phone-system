...
- **PATH CORRECTION (Expo SDK 57, discovered in Task 1):** the generated template's source root is **`mobile/src/`**, not `mobile/`. Therefore: our modules go in **`mobile/src/lib/`** (session.ts, api.ts, auth.tsx, theme.ts) and screens in **`mobile/src/app/`** — wherever this plan's later tasks say `mobile/lib/...` or `mobile/app/...`, read `mobile/src/lib/...` / `mobile/src/app/...`. The code's RELATIVE imports in the plan are already correct for this layout (`../lib/x` from `src/app/`, `../../lib/x` from `src/app/(tabs)/`, `./session` within `src/lib/`). BUT in the TEST files (which live in `mobile/__tests__/`), change every `../lib/X` to `../src/lib/X` — in both `import` statements AND `jest.mock(...)` calls. The template ships a FLAT `src/app/` (`index.tsx`, `explore.tsx`, `_layout.tsx`), not a `(tabs)/` group — adapt Task 5/6 to the actual generated structure. Also note: `mobile/tsconfig.json` has `"types": ["jest"]`; any future ambient `@types/*` must be added to that array. A `mobile/global.d.ts` CSS shim already makes `tsc --noEmit` exit 0 — keep it green.
